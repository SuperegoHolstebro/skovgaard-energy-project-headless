...
const Preview = React.memo(function Preview({
  ...
  const draft = previewState?.snapshot
  const published = previewState?.original
  ...
  const showPreview =
    schemaType?.icon || (typeof schemaType?.preview?.prepare === 'function' && !isLoading)
  ...
  if (type === 'title') {
    return showPreview && previewValues?.title ? <>{previewValues?.title}</> : <>{fallback}</>
  }

  if (type === 'subtitle') {
    return showPreview && previewValues?.subtitle ? <>{previewValues?.subtitle}</> : <>{fallback}</>
  }
