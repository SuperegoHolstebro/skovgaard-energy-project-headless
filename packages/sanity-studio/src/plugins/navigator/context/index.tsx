...
type State = Pick<NavigatorContextType, 'currentDir' | 'locale' | 'defaultLocaleId' | 'searchTerm'>
...
export const NavigatorProvider = ({
  ...
  const initialLocaleId = i18nEnabled ? (i18n.defaultLocaleId ?? i18n.locales[0].id) : undefined
  const initialState: State = {
    currentDir: new URLSearchParams(window.location.search).get(CURRENT_DIR_PARAM) ?? '',
    searchTerm: '',
    locale: !i18nEnabled
      ? undefined
      : (new URLSearchParams(window.location.search).get(CURRENT_LOCALE_PARAM) ?? initialLocaleId),
  }
  ...
  const filteredData = i18nEnabled
    ? data.filter(
        (page) => page.locale === state.locale || (!page.locale && i18n.requireLocale === false),
      )
    : data
...
function searchTree({ tree: root, searchTerm }: { tree: Tree; searchTerm: string }) {
  ...
  function searchInTree(tree: Tree) {
    for (const key in tree) {
      ...
      if (
        ...
        item.title.split(' ').some((word) => word.toLowerCase().startsWith(q)) ||
        item.slug
          ?.toLowerCase()
          .split('/')
          .some((word) => {
